...
#include "bn_format.h"
#include "bn_music.h"
#include "maxmod.h" // header diretto maxmod
...
void dog::update()
{
    ...
    if (jumping)
    {
        // Controllo aereo ridotto — stile Mario
        static constexpr bn::fixed AIR_ACCEL = bn::fixed(0.06); // più lento che a terra
        static constexpr bn::fixed AIR_FRICTION = bn::fixed(0.02); // decelerazione leggera
        static constexpr bn::fixed AIR_MAX_VX = bn::fixed(2.0);  // limite aria

        if (bn::keypad::right_held())
        {
            dir = DIR_RIGHT;
            chr_vx += AIR_ACCEL;
            if (chr_vx > AIR_MAX_VX) chr_vx = AIR_MAX_VX;
        }
        else if (bn::keypad::left_held())
        {
            dir = DIR_LEFT;
        ...
        {
            // Nessun input → leggera decelerazione aria
            chr_vx -= AIR_FRICTION;
    ...
    {
        // Controllo a terra — più reattivo
    ...
    // chr_vx è sempre positiva, dir ne determina il segno
...
public:
    ...
    static constexpr int GO_TO_BALL = 0; // si dirige verso la palla
    static constexpr int CARRY_BALL = 1; // porta la palla verso human
    static constexpr int DONE = 2; // ha consegnato, esce dallo schermo
    ...
    // Stesse costanti fisiche del dog principale
    static constexpr bn::fixed GROUND_ACCEL = bn::fixed(0.15);
    static constexpr bn::fixed GROUND_FRICTION = bn::fixed(0.08);
    ...
    // --- costruttore ---
    // skin_index: indice del dog scelto dal giocatore → l'helper usa uno diverso
    helper_dog(int player_skin,
        ball* _ball,
        human* _human,
        ...
        : ball_ptr(_ball), human_ptr(_human), enemies(_enemies), camera(_camera)
    {
        ...
        player = dog_item->create_sprite(
            chr_x - HALF_SCREEN_W,
            chr_y - CHR_FLOOR + CHR_FLOOR_HEIGHT, 0);
        player->set_bg_priority(2);
    ...
    // --- attivazione ---
    // Chiamata quando il giocatore perde la palla per la 3a volta
    ...
    // --- logica di movimento verso un target ---
    ...
    // --- salto automatico per schivare nemici ---
    void try_jump_over_enemies()
    {
        ...
        for (const enemy& e : enemies)
        {
            ...
            // Se c'è un nemico davanti nella direzione di marcia, entro 48px → salta
            ...
            if (in_front)
            {
                if (e.dir != dir) {
                    jumping = true;
                    chr_vy = HELP_JUMP_VY;
                    chr_vx = MAX_VX; // salto pieno per superarlo
                    walking = false;
                    break;
                }
                else
    ...
    // --- fisica e aggiornamento posizione ---
    void apply_physics()
    {
        // Orizzontale
        chr_x += bn::fixed(dir).multiplication(chr_vx);
    ...
    // Restituisce true se l'helper perde la palla
    ...
    {
        if (jumping)
            ; // nessuna animazione salto definita, resta sull'ultimo frame
        else if (walking)
            actionWalk->update();
        else
            actionStand->update();
    ...
    // --- update principale ---
    // Restituisce true quando ha consegnato la palla a human (stato DONE)
    bool update()
    {
        ...
        case GO_TO_BALL:
        {
            ...
            // Raccoglie la palla se ci è sopra
            bool on_ball = bn::abs(chr_x - ball_ptr->chr_x) < 16 &&
                !ball_ptr->pickedup;
            if (on_ball)
            {
                ball_ptr->pickedup = true;
            ...
            // Se ha perso la palla (non dovrebbe in GO_TO_BALL, ma per sicurezza)
        ...
        case CARRY_BALL:
        {
            if (lost_ball)
            {
                // Ha perso la palla durante il trasporto → torna a raccoglierla
                state = GO_TO_BALL;
                break;
            }

            // Porta la palla verso human
            move_toward(human_ptr->chr_x);
            try_jump_over_enemies();

            // Posiziona la palla sopra all'helper
            ...
            // Arrivato da human → consegna
            bool at_human = bn::abs(chr_x - human_ptr->chr_x) < 24;
            if (at_human)
            {
                ball_ptr->pickedup = false;
                ...
                return true; // segnala al chiamante che ha finito
            }
            break;
        }

        case DONE:
            // Esce dallo schermo verso destra
            dir = DIR_RIGHT;
            walking = true;
            chr_vx = MAX_VX;
...
{
    // I 5 sprite dog affiancati, centrati sullo schermo
    // Schermo GBA: -120 a +120 orizzontale
    // 5 sprite da 32px con 8px di margine = (32+8)*5 = 200px totali
    // Partenza: -80, poi +40 per ogni dog
    ...
    const bn::sprite_item* items[NUM_DOGS] = {
        &bn::sprite_items::dog,
        &bn::sprite_items::dog2,
        &bn::sprite_items::dog3,
        &bn::sprite_items::dog4,
        &bn::sprite_items::dog5
    };
    ...
    // Cursore: freccia/selettore sotto agli sprite
    // Usiamo uno sprite bau come cursore (o qualsiasi tile disponibile)
    bn::sprite_ptr cursor = bn::sprite_items::bau.create_sprite(START_X, CURSOR_Y, 0);
    //cursor.set_scale(bn::fixed(0.75));
    ...
    int selected = 0;
    int input_delay = 0; // antirimbalzo input
    ...
    while (true)
    {
        ...
        if (input_delay == 0)
        {
            if (bn::keypad::right_pressed() && selected < NUM_DOGS - 1)
            {
                selected++;
                input_delay = 12;
            }
            else if (bn::keypad::left_pressed() && selected > 0)
            {
                selected--;
                input_delay = 12;
            }
        ...
        // Evidenzia il dog selezionato con leggera scala
        for (int i = 0; i < NUM_DOGS; i++) {
            sprites[i]->set_scale(i == selected ? bn::fixed(1.5) : bn::fixed(1.0));
            sprites[i]->set_y(i == selected ? DOG_Y - 9 : DOG_Y); // solleva il selezionato
        }
...
int main()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            // Aggiornamento helper
            if (helper.active)
            {
                bool consegnata = helper.update();
                if (consegnata)
                {
                    //     palle_perse_lancio = 0;
                         // human resta in attesa — imposta un flag per la frase consolatoria
                     //    human.waiting_for_dog = true;
                     //    update_text(TESTO_CONSOLATORIO, &dog);
                }
            }

            if (bn::keypad::l_pressed() && !helper.active)
            {
                helper.spawn(dog.chr_x - 150); // appare vicino al cane
            }
