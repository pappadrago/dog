...
void live_obj::apply_rebound()
{
    ...
    // --- Fisica verticale ---
    {
        ...
        int ty = ((chr_y + (chr_vy > 0 ? box_dim : bn::fixed(0.0))).division(16.0)).floor_integer();
...
void live_obj::apply_map() {
    ...
    if (chr_x <= 0) { chr_x = bn::fixed(0);chr_vx = bn::fixed(0); }
    if (chr_x >= bn::fixed(MAP_W)) { chr_x = bn::fixed(MAP_W);chr_vx = bn::fixed(0); }
